...
// the daemon) issues a restricted asset whose policy key is generated by the
// DISTRIBUTED key generation — the default — then completes a hosted transfer
// whose policy co-signature the 2-of-3 quorum produced, and that witness
// signature verifies as plain BIP340 under the on-chain policy key. The node is
// a mock elementsd; nothing about enclaves, contracts, addresses or the wallet
// flow knows either that the backend is a quorum or that its key came from a
// DKG rather than a dealer.
...
// newE2EServer wires a policy server whose signer is selected exactly the way
// the daemon's -signer flag selects it: by name, through the registry, which is
// what makes this an end-to-end test of the wiring and not just of the package.
func newE2EServer(t *testing.T) (http.Handler, *store.Store, *e2eNode) {
	t.Helper()
	...
	return srv.Routes(), st, node
}

// fundE2E seeds the mock wallet with one explicit fee coin.
func fundE2E(node *e2eNode) {
...
func TestFrost_IssueAndTransferEndToEnd(t *testing.T) {
	...
	// The recorded policy key IS the DKG group key stored for the asset: a
	// 33-byte group point whose X is the on-chain x-only K_policy, one share per
	// member, and a pinned public share per member. Nothing here says "dealer":
	// this asset's key was generated distributively.
	keys, err := st.LoadKeys()
	if err != nil {
		t.Fatal(err)
	}
	groupPointHex, ok := keys["frost:"+assetID+":group"]
	if !ok {
		t.Fatalf("no frost group key bound to asset %s (keys: %v)", assetID, keys)
	}
	groupPoint := mustHexBytes(groupPointHex)
	if len(groupPoint) != 33 {
		t.Fatalf("group record must be a 33-byte compressed point, got %d bytes", len(groupPoint))
	}
	groupXHex := hex.EncodeToString(groupPoint[1:])
	for i := 1; i <= 3; i++ {
		for _, name := range []string{
			fmt.Sprintf("frost:%s:share:%d", assetID, i),
			fmt.Sprintf("frost:%s:pubshare:%d", assetID, i),
		} {
			if _, ok := keys[name]; !ok {
				t.Fatalf("missing %s for asset %s", name, assetID)
			}
		}
	}
	code, assetOk := do(t, h, "GET", "/v1/assets/"+assetID, "", nil)
	if code != 200 || assetOk["policy_pub"] != groupXHex {
		t.Fatalf("asset policy_pub %v != the group key's x-only form %s", assetOk["policy_pub"], groupXHex)
	}
	...
	pub, err := schnorr.ParsePubKey(mustHexBytes(groupXHex))
	if err != nil {
		t.Fatal(err)
	}
...
// TestFrost_DealerOverrideThroughTheDaemonPath covers the one configuration
// knob the daemon exposes for keygen: OPENAMPD_FROST_KEYGEN=dealer, read by the
// registry factory that -signer=frost goes through. The asset it issues must be
// indistinguishable on-chain from a DKG-keyed one — same single x-only
// K_policy, same enclave scripts — because the keygen construction is not
// something the chain can see.
func TestFrost_DealerOverrideThroughTheDaemonPath(t *testing.T) {
	t.Setenv("OPENAMPD_FROST_KEYGEN", "dealer")
	h, st, node := newE2EServer(t)
	fundE2E(node)
	...
	reg := func() string {
		...
		x := elements.XOnlyFromPriv(priv[:])
		code, out := do(t, h, "POST", "/v1/users", "", map[string]any{"pubkeys": []string{hex.EncodeToString(x[:])}})
		if code != 200 {
			t.Fatalf("register: %d %v", code, out)
		}
		return out["aid"].(string)
	...
	code, out := do(t, h, "POST", "/v1/issuer/assets", "tok", map[string]any{
		"name": "DEALERX", "ticker": "DEALERX", "precision": 8, "atoms": 1000,
		"holder_aid": reg(), "issuer_aid": reg(),
	})
	if code != 200 {
		t.Fatalf("dealer-mode issuance failed: %d %v", code, out)
	}
	assetID := out["asset"].(string)
	keys, err := st.LoadKeys()
	if err != nil {
	...
	groupPoint := mustHexBytes(keys["frost:"+assetID+":group"])
	if len(groupPoint) != 33 {
		t.Fatalf("group record must be a 33-byte compressed point, got %d bytes", len(groupPoint))
	}
	for i := 1; i <= 3; i++ {
		if _, ok := keys[fmt.Sprintf("frost:%s:share:%d", assetID, i)]; !ok {
			t.Fatalf("member %d has no share under the dealer override", i)
		}
	}
	code, asset := do(t, h, "GET", "/v1/assets/"+assetID, "", nil)
	if code != 200 || asset["policy_pub"] != hex.EncodeToString(groupPoint[1:]) {
		t.Fatalf("policy_pub %v is not the group key's x-only form", asset["policy_pub"])
	}
}
