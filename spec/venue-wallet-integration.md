...
Status of referenced platform work: the openampd and seqpald dependencies this document once marked as future (**M5**, **M6**, **M8**) are now deployed and live on the Sequentia testnet. The OA-7 cosign containment fix and the mixed-asset atomic-payment (OA-4) path landed in M6; the eligibility and listings endpoints landed in M8; the confidential (OA-8), redeem-burn (OA-5), reissue (OA-6), set-hash category log (OA-LM), and the M9 external issuer key plus two-phase clawback are all live. What remains outstanding is the WALLET-side and VENUE-side build itself: the SWK kit items (1.2), the web-wallet work items (1.9), and the SeqDEX settler. Those tags now identify which platform behavior a given surface depends on; every platform dependency named here is deployable against `https://sequentiatestnet.com/openamp` and `https://sequentiatestnet.com/seqpal/api` today.
...
The policy key is per-asset (each asset record carries its own `policy_pub`, bound at issuance in `issue.go` and read by `treeFor`, `server.go:421-430`), so a holder's enclave address differs per asset.

### 0.2 AID

`AID = hex(first 20 bytes of sha256("openamp-aid-v1" || pubkey-hex-strings sorted lexicographically, concatenated as UTF-8))` (`internal/store/store.go:567-577`, `func AID`). Pubkeys are 64-hex BIP340 x-only keys. The AID hashes the sorted pubkey SET: registering a different key, or a second key, produces a different AID. Clients must compute the AID locally and assert equality with the server's answer; a fixed cross-implementation test vector against the Go `store.AID` is part of wallet conformance.

### 0.3 Endpoint reference

All under `/openamp` on the live box (Caddy strips the prefix). Public unless noted.

- `POST /v1/users` `{pubkeys:["<64-hex x-only>", ...]}` -> `{aid}`. Idempotent; re-registering an existing AID leaves the server-side record (categories, frozen) untouched (`server.go:173-200`).
- `GET /v1/users/{aid}` -> `{aid, pubkeys, categories[], frozen}` (`server.go:202-215`). NOTE: `categories` and `frozen` are `omitempty` in the store struct (`store.go:22-23`), so they are ABSENT from the JSON when the list is empty or the user is not frozen. Clients must default them (no categories, not frozen), never treat absence as an error.
- `GET /v1/users/{aid}/address?asset=` -> `{aid, asset, address, script_pubkey, confidential, user_pubkey, transfer_leaf, transfer_control, claw_leaf?, claw_control?}` (`server.go:243-286`; `claw_leaf`/`claw_control` only when the asset has a clawback leaf). Confidential assets return a blech32 address.
- `GET /v1/users/{aid}/balance?asset=` -> `{aid, asset, atoms, utxos}`; `utxos` is a COUNT. Confirmed-only; no scan height, no pending amounts, no utxo detail (`server.go:288-306`, `enclaveUTXOs` `server.go:439-467`).
- `GET /v1/assets` and `GET /v1/assets/{id}` -> full asset record incl. `rules` and `contract` (restriction legend + terms_hash inside the contract's `openamp` block) (`server.go:217-241`).
- `GET /v1/supply?asset=<id>` -> `{asset, circulating_atoms, height}`. Purely chain-derived circulating supply (the sum of confirmed enclave holdings), so a redeem-burn (OA-5) lowers it and a reissue (OA-6) raises it once confirmed (`issue.go:709-739`). Public.
- `POST /v1/transfers` (hosted builder) and `POST /v1/transfers/{id}/complete` -> see 1.6.
- `POST /v1/cosign` (self-built transactions) -> see 2.4.
- `GET /v1/log` -> hash-chained transparency log (registrations, refusals, cosigns, transfers, burns, anchors). OA-LM: a `categories` entry records `categories_hash` (a commitment to the category SET, format `"v":1`), not the raw category vector, so the public log stops leaking each holder's exact categories; the private policy state keeps the raw set (`server.go:358-367`).
- Bearer-token-gated `/v1/issuer/*` endpoints exist for the platform only; wallets and venues never hold that token. They include the M9 external-issuer two-phase clawback (`POST /v1/issuer/clawback` builds the sweep, `POST /v1/issuer/clawback/{id}/complete` broadcasts after the issuer's own signature) and the redeem-burn/reissue builders (`server.go:120-129`).

### 0.4 Signing formats (normative for every client)

1. **Enclave-spend signatures are plain (untagged) BIP340 over the Elements taproot sighash**: SIGHASH_DEFAULT, committing to the genesis hash and the transfer leaf (`elements.TaprootSighash`, called at `transfer.go:706` in the hosted build and `transfer.go:1095` in cosign). The server supplies its computed digest in `to_sign`, but the wallet MUST recompute the digest itself from the transaction and prevouts and sign its OWN result, refusing on mismatch (see 0.4(3); a supplied digest is a cross-check, never the thing you sign). Signatures must verify under the FIRST registered pubkey of the sender's AID (`/complete` verifies against `sender.Pubkeys[0]`, `transfer.go:867-885`; the enclave tree is built from `user.Pubkeys[0]`, `server.go:421-430`).
...
5. **Wire encoding**: `atoms` is a JSON NUMBER (uint64), never a string; openampd decodes the request field into a bare Go `uint64` and rejects strings (`transfer.go:426-430`). Sig maps in `/complete` are keyed by input index as a DECIMAL STRING (`transfer.go:833-886`). Signatures are 128-hex (64 bytes); pubkeys are 64-hex x-only.
...
3. Freeze is GLOBAL per user across every asset on the instance (`server.go:312-335`).
4. Pending hosted-transfer builds are PERSISTED and survive a restart, with a 72-hour TTL (`server.go:68-72` `pendingTTL`; `PutPendingTransfer`/`GetPendingTransfer`, `transfer.go:716-735`). The in-memory map is a fast-path cache over the durable store. The longer, restart-durable TTL exists because an atomic OA-4 settlement can involve several parties who each sign at their own pace. A draft is consumed exactly once on `/complete` (delete-before-use, `transfer.go:806-831`), so a completed or replayed id 404s: rebuild on 404, never blind-retry.
...
8. `/v1/cosign` velocity-counts approvals even if never broadcast: it writes a `TransferRecord` and a `cosign` log entry on approval (`transfer.go:1110-1117`), and `checkTransfer`'s velocity window sums those records (`transfer.go:160-174`), so never cosign for quotes.
9. Recipients must pre-exist: a restricted output to an out-of-enclave destination is refused (`transfer.go:62-64`), and a transfer names recipients by AID, which `enclaveFor` 404s if unregistered (`server.go:398-419`), so a plain address or an unregistered party cannot receive the asset; receive UX leads with "share your AID".
...
- Transparent-by-default: enclave addresses are ordinary tb1 P2TR; blech32 only when a holder elects confidentiality for a transfer, presented as opt-in.
...
1. `POST /v1/transfers {"asset", "sender_aid", "recipient_aid", "atoms": <JSON number>, "fee_mode": "convert"}` -> `{id, tx, to_sign:[{input, sighash, pubkey}], convert_atoms, fee_sats}`. Expect: 404 unregistered recipient, 409 insufficient confirmed balance, 503 policy fee wallet dry (`handleTransferBuild`, `transfer.go:425-747`). The response also carries `payment_inputs` when an optional atomic OA-4 payment leg was requested (2.4); a plain wallet send omits it, so this response is byte-identical to the single-party flow.
...
5. `POST /v1/transfers/{id}/complete {"sigs": {"<input index>": "<128-hex>"}}` -> `{txid}`; on 403 surface the reason VERBATIM and link the transparency log; on 404 the draft was consumed or expired (72 h TTL, 0.5(4)): rebuild, never blind-retry.
6. Track the txid per 1.5(b).

**Known interop bug (fix + contract-test in the web wallet):** the shipped send path posts `atoms` as a JSON string (`index.html:2980`), which openampd's `uint64` decode rejects (`transfer.go:426-430`). Verify against the live server, then fix to a number.
...
2. **Buyer eligibility:** buyer categories (`GET /v1/users/{aid}`) must match asset rules (`GET /v1/assets/{id}`). Categories are stamped only by the platform; SeqDEX cannot grant them. An advisory preflight `GET /seqpal/api/eligibility?aid=&asset=` -> `{aid, asset, eligible, reasons[]}` is live (it checks the buyer's freeze state, whether they hold a permitted `allowed_categories` token, and the Reg S category-deny windows against the chain tip; `seqpald/id.go` `handleEligibility`, `evalEligibility`). It does not re-derive holder caps, vesting, or velocity, which are chain-balance dependent and left to the policy server at co-sign time. Both query parameters are required (400 if absent); an unregistered AID or unknown asset returns `eligible:false` with a reason, not an error. Treat it as advisory; the policy server's refusal at co-sign time is the actual gate.
3. **Listing authorization:** markets for restricted assets are issuer-authorized. seqpald exposes `GET /seqpal/api/listings` (public; `seqpald/listings.go` `handleListings`). Without a query it returns `{"listings": [ <listing> ... ], "note": "..."}` of the AUTHORIZED listings, optionally filtered by `?issuer=<aid>`. With `?asset=<id>` it returns `{"listing": <listing>, "authorized": true}` for an authorized asset, or `{"asset": "<id>", "authorized": false, "note": "..."}` when there is no authorization on record. A `<listing>` object is `{asset, issuance_id?, issuer_aid?, ticker?, name?, authorized, venues, signature?, signer_xonly?, created_at, updated_at}` (`authorized` is the boolean, not `issuer_authorized`; `venues` is a JSON array of the venues the issuer scoped the grant to). The restriction legend and `terms_hash` are NOT on the listing object; read them from the asset's contract `openamp` block via `GET /openamp/v1/assets/{id}`. The issuer grants the authorization from their platform account (`POST /api/issuances/{id}/listing`, optionally signed with the issuer key); the venue checks at market creation. Display the restriction legend on the market page.
4. **Asset metadata:** use the Sequentia asset registry for display (ticker, name, precision, contract with the openamp block).
5. **Blinded coins are excluded:** `/v1/cosign` refuses any blinded output or blinded enclave input, whatever the asset; a blinded holding settles through the hosted flow only (atomic swap legs are transparent-only).
...
Settlement of a fill is a single atomic swap transaction with a restricted leg and a payment leg, routed through `POST /v1/cosign` (`handleCosign`, `transfer.go:995-1119`):
...
The settler builds the transaction: inputs = seller's enclave UTXOs (restricted asset) + buyer's ordinary UTXOs (payment asset + fee); outputs = restricted asset to the BUYER'S REGISTERED ENCLAVE for this asset (script from `GET /v1/users/{aid}/address?asset=`), payment asset to the seller's ordinary address, changes, one explicit fee output. Constraints enforced by openampd: **the ENTIRE transaction must be transparent** (every output explicit asset and value, not only the restricted leg: `checkTransfer` rejects any confidential asset or value commitment anywhere in the tx, `transfer.go:43-49`), so the payment leg, all change, and the fee output must be unblinded; a restricted output in a fee (empty-scriptPubKey) output is refused (`transfer.go:54`); every restricted output must pay a registered user's enclave for this asset (OP_RETURN allowed only if the contract has burn_allowed, `transfer.go:56-61`); exactly one fee asset per transaction (Sequentia mempool rule); sender not frozen; every recipient not frozen and holding an allowed category; lockin/vesting/velocity/holder-cap/category-deny rules all apply. The OA-7 containment loop (2.5) additionally refuses any enclave input of this asset that is not in the claimed set. On approval, openampd returns policy signatures WITHOUT broadcasting: `{"sigs":[{"input","sighash","policy_sig","leaf","control"}]}`. The settler then: collects the seller's BIP340 signatures over each returned sighash (2.3), assembles enclave witness stacks as `[policy_sig, user_sig, leaf, control]` bottom to top, has the buyer sign their own payment/fee inputs normally, and broadcasts.

Known behaviors to design around: (a) a cosign writes a TransferRecord and a log entry EVEN IF the tx is never broadcast, and velocity counts it (`transfer.go:1110-1117`): do not cosign for quotes; (b) pending hosted-transfer state is irrelevant here (cosign is stateless); (c) prevouts are resolved mempool-inclusive (`getrawtransaction` with an electrs fallback for confirmed prevouts when the node lacks -txindex, `transfer.go:348-379`), but policy checks (velocity, holder caps, balances) run against confirmed chain state, so settling from unconfirmed enclave outputs is discouraged; verify against `handleCosign`/`spentOutputs` before relying on either behavior.

### 2.4b Paying in BTC (native, parent chain): what is possible and what is not

Buyers can and should be able to pay in BTC. Every Sequentia wallet is dual-chain and already holds native Bitcoin (testnet4), Sequentia has NO pegged BTC by design, and eligibility is orthogonal to payment: the policy server constrains WHO may hold the asset (a registered, eligible AID), never WHAT the buyer paid with. But the shape of a BTC trade is constrained by one hard rule.

**A restricted asset can never sit in an HTLC output.** Every output carrying the asset must land on a REGISTERED ENCLAVE scriptPubKey or be refused ("restricted asset to an out-of-enclave destination", `transfer.go:64`; the only other permitted form is an OP_RETURN burn when `burn_allowed`). A hash-locked swap output is not an enclave, so the classic cross-chain HTLC swap used today for BTC against ordinary assets CANNOT be applied to the restricted leg. Do not attempt it.

Workable routes, in ascending order of ambition:

1. **Two hops, each atomic on its own (buildable on existing rails).** BTC -> ordinary asset (for example USDX) as a standard SeqDEX cross-chain atomic swap, then ordinary asset -> restricted asset as the single-transaction, policy-co-signed DvP of 2.4. Each hop is individually atomic; between hops the buyer holds an ordinary asset, so the residual exposure is price, not counterparty. A venue may present this as one "pay with BTC" action that routes automatically, provided the routing and the intermediate state are disclosed honestly.
2. **Registrar-style settlement.** BTC payment observed and confirmed on Bitcoin testnet4, then policy-co-signed delivery on the Sequentia network. Not atomic: someone carries the gap between the two chains (in SeqPal's primary market that party is the registrar, and the trust difference is surfaced in the UI). On a secondary venue the venue or a liquidity provider carries it, and that must be stated on the market page.
3. **Liquidity provider fronting BTC.** An LP quotes BTC and internally runs route 1 or 2. CONSEQUENCE, easy to miss: any LP or market maker that ever HOLDS restricted-asset inventory is itself a holder, so it must be a registered, eligible AID with an appropriate category. Liquidity in these assets cannot be provided anonymously, and a venue must not assume otherwise.

**Open design item: true atomicity via adaptor signatures (recommended investigation, not yet specified).** Hashlocks are unavailable, but Schnorr adaptor signatures need no script support. An enclave spend needs a policy signature and a user signature, and `POST /v1/cosign` already returns the policy signature for a specific transaction WITHOUT broadcasting it (2.4). So the seller can supply an ADAPTOR signature for their user half, locked to the same secret as the Bitcoin-side leg: when the seller claims the BTC they reveal the secret, which completes the seller's signature and lets the buyer broadcast the Sequentia transfer. That is a genuinely atomic BTC-to-restricted-asset swap with no change to the enclave script and no new openampd endpoint. Settle these before building, because they are the design, not details: (a) a cosign writes a velocity-counted TransferRecord even if the transaction is never broadcast (`transfer.go:1110-1117`), so abandoned swaps distort velocity accounting; (b) a pre-cosigned transaction carries no expiry, so a counterparty could sit on a completed signature and broadcast it after eligibility or freeze state has changed, and the policy server cannot revoke a signature it already issued: the only cancel is the seller double-spending their own enclave UTXO, which must therefore be part of the protocol, alongside a UTXO-locking or transaction-expiry convention; (c) the full failure and refund matrix across both chains, including a Bitcoin-side timeout after the policy signature was issued.

### 2.5 Required openampd precondition (named explicitly)

**The `/v1/cosign` unclaimed-enclave-input containment fix (OA-7): DEPLOYED.** `handleCosign` now runs a containment loop (`transfer.go:1045-1069`): it resolves the enclave scriptPubKey of every registered user for this asset, then refuses any UNCLAIMED input landing on one ("input N is an unclaimed enclave output of this asset", HTTP 403, logged). Claimed inputs are additionally checked to be the sender's own enclave output ("input N is not the sender's enclave output", `transfer.go:1040-1043`). This is exercised by `internal/server/oa7_test.go` (`TestOA7_RejectsUnclaimedEnclaveInput` and `TestOA7_AllowsFullyClaimedCosign`). Settlement on `/v1/cosign` is now safe to build.

**Mixed-asset tolerance: PROVEN.** `checkTransfer` iterates the outputs, `continue`s past any output whose asset commitment is not this restricted asset (`transfer.go:50-52`), and enforces the restricted-leg rules only on the restricted outputs, so a foreign payment leg (for example an explicit USDX output) alongside the restricted leg in one transaction is accepted, provided the WHOLE transaction is transparent (`transfer.go:43-49`). This is proven by `internal/server/oa4_test.go` (`TestOA4_MixedAssetProof`, and `TestOA4_MixedAssetProof_ConfidentialPaymentOutputRejected` for the transparency rule) and by seqpald's `m6_test.go`. The server also builds and settles the mixed-asset leg itself through the `/v1/transfers` OA-4 payment leg (2.4); a venue that self-builds can instead route the assembled two-asset transaction through `/v1/cosign`.
...
- **HTLC swaps of the restricted leg:** structurally impossible (2.4b). BTC payment is supported, but never by hash-locking the restricted asset.
...
- `GET /seqpal/api/eligibility?aid=&asset=` -> `{aid, asset, eligible, reasons[]}`. Advisory preflight, live. Both params required.
- `GET /seqpal/api/listings` -> `{listings:[...]}` (or `?asset=<id>` -> `{listing, authorized}` / `{asset, authorized:false}`; `?issuer=<aid>` filters). Listing gate, live. Legend and `terms_hash` come from the asset contract, not this response.
...
**A venue conforms when:** it lists only issuer-authorized restricted markets with the legend displayed; it supports BTC payment by one of the routes in 2.4b and discloses that route's atomicity and trust properties honestly, never hash-locking the restricted leg; gates placement on AID-control proof and balance; treats preflights as advisory and 403s as normal outcomes surfaced verbatim with log links; settles per 2.4 only after OA-7 is confirmed deployed and the mixed-asset proof exists; sends sellers full transactions plus prevouts/leaf/control, never bare digests; never labels anything final at 0-conf; retains counterparty fill records; carries the 2.7 regulatory notes on market pages.

## Part 4. Dependency summary

The platform-side dependencies (openampd, seqpald) are all DEPLOYED and live on testnet; what remains is the wallet-side and venue-side build.

| Dependency | Where | Status | Blocks |
|---|---|---|---|
| OA-7 cosign containment fix | openampd (M6) | DEPLOYED (`transfer.go:1045-1069`, `oa7_test.go`) | all venue settlement (2.4) |
| Mixed-asset atomic-payment proof | openampd `oa4_test.go` + seqpald `m6_test.go` | DEPLOYED (2.5) | venue settlement confidence (2.5) |
| Sign card + enclave-spend signing surface | sequentia-web-wallet (M5; WW-2) + SWK-6 | wallet-side, OUTSTANDING | venue seller channel (a); platform wallet-link |
| Eligibility + listings endpoints | seqpald (M8) | DEPLOYED (`/seqpal/api/eligibility`, `/seqpal/api/listings`) | venue preflight + listing gate (2.2) |
| SWK-1..4 | SWK | wallet-side, OUTSTANDING | wallet conformance (Part 1); SWK-4 also unblocks registry verification of OpenAMP assets |
| m/5/0 migration (WW-1) | sequentia-web-wallet | wallet-side, OUTSTANDING | the sign card must NOT ship on the m/3/0 key |
